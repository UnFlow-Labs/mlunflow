...
new_training_config = TrainingConfig(
    num_epochs=5, optimizer="AdamW", optimizer_params={"lr": 0.001, "weight_decay": 0.01}
)
...
new_training_config_longer_epochs = TrainingConfig(
    num_epochs=10, optimizer="AdamW", optimizer_params={"lr": 0.001, "weight_decay": 0.01}
)
new_result_longer_epochs = run_experiment(new_training_config_longer_epochs)
print(
    f"New Result with AdamW and longer epochs: Loss = {new_result_longer_epochs['loss']:.4f}, Accuracy = {new_result_longer_epochs['accuracy']:.2f}%"
)
...
new_training_config_leakyrelu = TrainingConfig(
    num_epochs=10,
    optimizer="AdamW",
...
print(
    f"New Result with LeakyReLU: Loss = {new_result_leakyrelu['loss']:.4f}, Accuracy = {new_result_leakyrelu['accuracy']:.2f}%"
)
...
new_training_config_elu = TrainingConfig(
    num_epochs=10,
    optimizer="AdamW",
    optimizer_params={"lr": 0.001, "weight_decay": 0.01},
    model_settings=ModelConfig(
...
new_training_config_tanh = TrainingConfig(
    num_epochs=10,
    optimizer="AdamW",
    optimizer_params={"lr": 0.001, "weight_decay": 0.01},
    model_settings=ModelConfig(
...
new_training_config_muon = TrainingConfig(
    num_epochs=10, optimizer="Muon", optimizer_params={"lr": 0.001, "weight_decay": 0.01}
)
...
for batch_size in [32, 64, 128, 1024, 2048]:
    new_training_config_batchsize = TrainingConfig(
        num_epochs=10,
        optimizer="AdamW",
        optimizer_params={"lr": 0.001, "weight_decay": 0.01},
        model_settings=ModelConfig(
    ...
    print(
        f"New Result with batch size {batch_size}: Loss = {new_result_batchsize['loss']:.4f}, Accuracy = {new_result_batchsize['accuracy']:.2f}%"
    )
