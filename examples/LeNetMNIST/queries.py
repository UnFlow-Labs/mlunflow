...
def activation_change_exists(transformation):
    ...
    return any(
        get_change_field(change, field_name) is not None 
        and change["to_value"] == "Tanh"
...
for transformation in changes:
    ...
    if change_in_accuracy > 0:
        ...
        print(f"  - Activation function changed from {transformation.args_changes['activation_fn'].get('from_value')} to {transformation.args_changes['activation_fn'].get('to_value')}")   
        print(f"Change in accuracy: +{change_in_accuracy:.4f} from {source_outcome['accuracy']:.2f}% to {target_outcome['accuracy']:.2f}%")
    else:
        print(f"Transformation from {source_state.name} to {target_state.name}:")
        print(f"  - Activation function changed from {transformation.args_changes['activation_fn'].get('from_value')} to {transformation.args_changes['activation_fn'].get('to_value')}")
        print(f"Change in accuracy: {change_in_accuracy:.4f} from {source_outcome['accuracy']:.2f}% to {target_outcome['accuracy']:.2f}%")